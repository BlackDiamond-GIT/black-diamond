...
class CloudinaryImageAdmin(BDModelAdmin):
    list_display = (
        "thumb_preview",
        "title",
        "public_id",
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context["therapists_list"] = list(
            Therapist.objects.order_by("sort_order", "name").values("id", "name")
        )
        return super().changelist_view(request, extra_context=extra_context)

    @admin.display(description=_("Assigned to"))
    def therapist_assignment(self, obj: CloudinaryImage) -> str:
        parts = []
        for m in obj.therapist_main.all():
            parts.append(
                format_html(
                    '<span style="display:inline-block;background:#0ABAB5;color:#fff;'
        ...
        for m in obj.therapist_gallery.all():
            parts.append(
                format_html(
                    '<span style="display:inline-block;background:#00827F;color:#fff;'
                    'font-size:0.7rem;padding:1px 6px;border-radius:3px;margin:1px">'
                    "{} — {}</span>",
                    m.name,
                    _("gallery"),
    ...
    def assign_btn(self, obj: CloudinaryImage) -> str:
        ...
        assignments = []
        for m in obj.therapist_main.all():
            assignments.append({"therapist_id": m.pk, "therapist_name": m.name, "type": "main"})
        for m in obj.therapist_gallery.all():
            assignments.append({"therapist_id": m.pk, "therapist_name": m.name, "type": "gallery"})

        return format_html(
            '<button type="button" class="cld-assign-btn" data-image-id="{}" data-assignments="{}" '
            'style="cursor:pointer;background:#0ABAB5;color:#fff;border:none;'
